        ...
        if "formats" in response_json:
            for formats in response_json["formats"]:
                ...
                if format_string is not None and not "audio only" in format_string:
                    ikeyboard = [
                        InlineKeyboardButtonInlineKeyboardButton(
                "🎥 Video", callback_data=f"(cb_string_video).encode("UTF-8")||{format_ext}||{format_string}||{approx_file_size}"), InlineKeyboardButton("Document",
                                                                                                    callback_data=f"(cb_string_file).encode("UTF-8")||{format_ext}||{format_string}||{approx_file_size}")
                    ]
